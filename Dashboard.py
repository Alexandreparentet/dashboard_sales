...
def fomata_numero(valor, prefixo = ''):
    for unidade in ['', 'mil']:
        if valor < 1000:
...
# fig_pie_categorias = px.pie(vendas_categorias,
#                                values = 'Preço',
#                                names = vendas_categorias.index,
#                                title = 'Pie por Categoria')
...
fig_venda_mensal = px.line(vendas_mensal,
                           x = 'Mês',
                           y = 'Preço',
                           markers = True,
...
fig_venda_estados = px.bar(vendas_estados.head(),
                           x = 'Local da compra',
                           y = 'Preço',
                           text_auto = True,
                           title = 'Top 5 estados')
...
with aba1:
    ...
    with coluna1:
        st.metric('Receita', fomata_numero(dados.Preço.sum(), 'R$'))
        st.plotly_chart(fig_mapa_receita, use_container_width = True)
        st.plotly_chart(fig_receita_estados, use_container_width = True)
    with coluna2:
        st.metric('Quantidade de Vendas', fomata_numero(dados.shape[0]))
...
with aba2:
    ...
    with coluna1:
        ...
        # st.plotly_chart(fig_pie_categorias, use_container_width = True)
    with coluna2:
        st.metric('Quantidade de Vendas', fomata_numero(dados.shape[0]))
...
with aba3:
    qtd_vendedores = st.number_input('Quantidade de vendedores', min_value = 2, max_value = 10, value = 5) # 2, 10, 5
    coluna1, coluna2 = st.columns(2)
    with coluna1:
        st.metric('Receita', fomata_numero(dados.Preço.sum(), 'R$'))
